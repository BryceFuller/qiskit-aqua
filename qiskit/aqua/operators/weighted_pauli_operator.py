...
class WeightedPauliOperator(BaseOperator):
    ...
    def construct_evaluation_circuit(self, wave_function, statevector_mode,
                                     ...
                                     snapshot_individual_terms=False, circuit_name_prefix=''):
        """
        ...
        instructions = self.evaluation_instruction(statevector_mode, use_simulator_snapshot_mode, snapshot_individual_terms)
        circuits = []  
        if use_simulator_snapshot_mode:
            ...
            if snapshot_individual_terms:
                ...
                for i in range(len(self.paulis)):
                    instr = instructions.get('expval_snapshots_{}'.format(i), None)
        ...
        if use_simulator_snapshot_mode and self.paulis:
            ...
            if snapshot_individual_terms:
                ...
                for i, pauli in enumerate(self.paulis):
                    snapshot = SnapshotExpectationValue('expval_term_{}'.format(i), [pauli], variance=True)
    ...
    def evaluate_with_result(self, result, statevector_mode, use_simulator_snapshot_mode=False, 
                             snapshot_individual_terms=False, circuit_name_prefix='', simulated_shots=None):
        """
        ...
        if use_simulator_snapshot_mode:
            ...
            if snapshot_individual_terms:
                ...
                stdevH = 0
                for i, pauli in enumerate(self.paulis):
                    expval = snapshot_data['expectation_value']['expval_term_{}'.format(i)][0]['value']
                    expval = expval[0] + 1j * expval[1]
                    avg += expval 
                    P = np.real(expval / pauli[0])
                    deltaP = (1-P**2)
                    if deltaP < 0:
                        deltaP = 0
                    #There is an ambiguity to be addressed here, that simulated shots is being interpreted
                    # to mean the # of shots used to evaluate each operator term. In practice, the total # 
                    # of shots used for each energy evaluation will be simulated_shots * the min number of 
                    # noncommuting sets of states. 
                    stdevH += np.real(pauli[0]) * np.sqrt(deltaP / simulated_shots)
                    shot_noise = np.random.normal(0, np.abs(stdevH))
                avg += shot_noise
